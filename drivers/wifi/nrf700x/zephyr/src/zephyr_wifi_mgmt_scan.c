			   ...
			   scan_result_cb_t cb)
{
	...
	if (params) {
		...
		for (i = 0; i < WIFI_MGMT_SCAN_CHAN_MAX_MANUAL; i++) {
	...
	if (params) {
		...
		for (i = 0; i < WIFI_MGMT_SCAN_CHAN_MAX_MANUAL; i++) {
			struct wifi_band_channel *bc = &params->band_chan[i];

			if (bc->channel == 0) {
				break;
			}

			if (bc->band == NRF_WIFI_BAND_INVALID) {
			...
			band = nrf_wifi_map_zep_band_to_rpu(bc->band);
